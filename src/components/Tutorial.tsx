...
const tutorialCopy = {
  zh: {
    ...
    piecesIntro: "在正规棋盘上依次完成七类棋子的移动和吃子示例，结合九宫、河界与真实落点理解规则。", tapTarget: "点击一个金色落点", practiced: "很好，这一步走对了", nextPiece: "下一个棋子", finishPieces: "完成棋子练习",
  ...
  en: {
    ...
    piecesIntro: "Complete a movement and capture example for every piece on a full Xiangqi board, using the real palaces, river, and intersections.", tapTarget: "Tap a gold destination", practiced: "Nice—this move is correct", nextPiece: "Next piece", finishPieces: "Finish piece practice",
...
function PieceLesson({ t, language, pieceStyle, pieceTheme, onComplete }: { t: TutorialText; language: Language; pieceStyle: PieceStyle; pieceTheme: PieceTheme; onComplete: () => void }) {
  ...
  function loadScenario(nextPieceIndex: number, nextExampleIndex: 0 | 1) { const nextScenario = pieceScenarios[pieceOrder[nextPieceIndex]][nextExampleIndex]; setPieceIndex(nextPieceIndex); setExampleIndex(nextExampleIndex); setPieces(nextScenario.pieces); setSelectedId(nextScenario.actorId); setPracticed(false); setLastMove(null); }
  function move(position: Position) {
    ...
    setPieces(next); setLastMove({ from: { row: selectedPiece.row, col: selectedPiece.col }, to: position }); setSelectedId(null); setPracticed(true);
  }
  function next() {
    if (exampleIndex === 0) { loadScenario(pieceIndex, 1); return; }
    if (pieceIndex === pieceOrder.length - 1) { onComplete(); return; }
    loadScenario(pieceIndex + 1, 0);
  }
  return <>
    ...
    <div className="piece-lesson-layout">
      ...
      <div className="piece-lesson-copy">
        ...
        <div className="piece-example-progress"><span className={exampleIndex === 0 ? "is-active" : "is-done"}>1</span><i /><span className={exampleIndex === 1 ? "is-active" : ""}>2</span><small>{t.exampleProgress}</small></div>
        ...
        <button type="button" onClick={next} disabled={!practiced}>{exampleIndex === 0 ? t.nextCapture : pieceIndex === pieceOrder.length - 1 ? t.finishPieces : t.nextPiece} →</button>
      </div>
    </div>
  </>;
}
